...
#### Миграция существующего xHTTP

1. Обновите локальные скрипты и загрузите их на сервер обычным способом.
2. Откройте **VLESS Reality → Добавить xHTTP** и подтвердите переустановку.
3. Дождитесь успешной проверки Mihomo, nginx и реального xHTTP probe.
4. Обновите клиентские конфиги и опубликованные подписки: новый xHTTP использует TLS fingerprint `firefox`.

Переустановка сохраняет текущую топологию, порты, path, обычных и cascade-пользователей, routing и sub-rules. Для варианта с nginx внешний TLS остаётся на nginx, а внутренний канал к Mihomo переводится на HTTP. При ошибке скрипт восстанавливает прежние файлы и состояние сервисов. TCP и gRPC не меняются. Миграция запускается только после явного подтверждения пользователя.
